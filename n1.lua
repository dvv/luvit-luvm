...
--
function install(root, dir, continue)

  Fiber.new(function (resume, wait)
    local err, result

    -- module manifest exists?
    Fs.stat(Path.join(dir, 'config.lua'), resume)
    err = wait()

    -- manifest is present, process it
    if not err then
      local config = require(Path.join(dir, 'config'))
      --p(config)
...
print('wgot', err)
          ...
          if #result == 1 then
print('ren1?', Path.join(tmp_mod_path, result[1]), mod_path)
...
print('ren2?', tmp_mod_path, mod_path)
    ...
    -- build this module
    Fs.stat(Path.join(dir, 'Makefile'), resume)
    err = wait()
    if not err then
      shell('make -C ' .. dir, resume)
      err = wait()
    end
...
--
install('/home/dvv/LUA/MOD/N', '/home/dvv/LUA/MOD/N', function()
--install('.', function()
  p('DONE')
end)
